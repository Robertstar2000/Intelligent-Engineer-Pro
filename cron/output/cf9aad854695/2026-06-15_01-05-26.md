...
[MEMORY PALACE QUERY FAILED] MemPalace query failed: ModuleNotFoundError: No module named 'sentence_transformers'. Install dependencies to enable vector memory search.

[LOCAL BACKUP FAILED] Backup script timed out during rsync of .hermes directory (after 60 seconds). The .hermes directory appears to be large (multiple gigabytes). Increase timeout or consider excluding additional directories to reduce backup size.

[GITHUB STEP SKIPPED] Skipped GitHub backup due to local backup failure.

For troubleshooting:
- Check USB mount: mountpoint -q /mnt/usb_4tb
- Verify available space on backup destination
- Consider adjusting exclusions in hermes-backup skill (e.g., exclude large skill directories temporarily)
- Manual backup command: hermes backup (if not running as cron) with increased timeout settings

Backup did not complete successfully. No archive was created.
