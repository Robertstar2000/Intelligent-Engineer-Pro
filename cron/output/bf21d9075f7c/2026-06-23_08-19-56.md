...
- **Manual orchestrator fallback (when script unavailable)** — If `daily-pipeline-analysis.py` is missing or broken, the 7-step orchestrator can be run manually using inline Python via `terminal()`. The critical computation is days-in-stage, which requires handling two date formats: Books/SaaS use ISO timestamps (`datetime.fromisoformat(s.replace('Z', '+00:00'))`) while Consulting uses date-only strings (`datetime.strptime(s, '%Y-%m-%d').replace(tzinfo=timezone.utc))`). Always anchor `now` to a fixed datetime for reproducibility (e.g., `datetime(2026, 6, 22, 8, 0, 0, tzinfo=timezone.utc)`). The report should follow the template at `references/report-format-2026-06-20.md` — overview table, per-pipeline detail with days-in-stage, blocker summary, email queue, 7-day projection, registry integrity cross-reference, and prioritized recommended actions.
...
📊 **Daily Pipeline Health Report — 2026-06-23 (Tuesday)**

All 7 orchestrator steps completed. Report saved to `data/daily-pipeline-report.md`.

---

**📈 Overview:** 16 leads across 3 pipelines | $1,653.93 value (excl. SaaS) | 9 blockers | 0 auto-advances

**✅ Registry Integrity:** PASS — all 3 pipelines match (Books: 3/3, SaaS: 5/5, Consulting: 8/8, Total: 16/16)

---

**🔴 Top Issues:**

1. **Consulting pipeline is fully stalled** — All 8 leads exceed the 7-day blocker threshold (47 days idle). 7 of 8 have had zero outreach since the May 7 fresh start. Three leads are confirmed/likely real with emails ready: C-07 Kris Simpson (Crestwood Municipal), C-06 Dr. Luis Dorado (Harbor CC), C-03 Gregory Shepherd (Meridian Financial).

2. **Books B-002 & B-003 idle 46 days** — No outreach attempted since BookExpo referral and outbound sales contact. B-003 has a critical domain verification flag (thebookcellar.com shows "FOR SALE").

3. **SaaS S-001 (Sarah Chen, ICP 92)** — 39 days in Contacted stage with no advancement. Highest ICP score in the pipeline.

**⚠️ Nurture Status:**
- Books: Cindy Lou Legal Capers (3 draft titles) missing from nurture sequence — low severity since unpublished
- SaaS: ✅ All products covered
- Consulting: Product names not explicitly matched in nurture body text (conceptually covered)

**📧 Email Queue:** 6 emails queued but ON HOLD due to nurture gaps. Recommend approving sends for consulting leads C-003, C-006, C-007 (confirmed entities with emails) and S-001 follow-up.
