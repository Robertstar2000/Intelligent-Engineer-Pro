...
9. **Never use `hermes config set` with JSON strings.** The command mangles JSON-in-YAML (splits on commas, produces 1500+ string fragments). For list values like `skills.disabled`, edit config.yaml directly using Python's `yaml` module:
   ...
   cfg['skills']['disabled'] = ['skill-a', 'skill-b']  # proper Python list
   with open(os.path.expanduser('~/.hermes/config.yaml'), 'w') as f:
       yaml.dump(cfg, f, default_flow_style=False, allow_unicode=True)
   ```

10. **`patch` tool can leave orphaned code.** When replacing a function body, the old body may survive if `old_string` doesn't match exactly. Always verify with `grep -n` after patching. If the old code remains, do a second patch to remove it, or use `write_file` for the entire function.
