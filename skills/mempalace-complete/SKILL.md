...
#### Dependency Installation in Isolated Environments
- When installing MemPalace dependencies in cron jobs or isolated environments, network issues may cause pip install failures
- **Troubleshooting steps for dependency installation:**
  1. Use `uv pip install` with `--no-cache-dir` flag to avoid cache-related issues
  2. Install packages individually if bundled installs fail: `pip install numpy`, then `pip install sentence-transformers`, then `pip install faiss-cpu`
  3. Use `--no-deps` flag to install packages without dependencies first, then install dependencies separately
...
**Maintenance Script Fixes for Cron Jobs:**
When creating or fixing maintenance scripts like `cron_maintenance.py` for cron job execution:
1. **Convert relative imports to absolute imports** - Change `from . import module` to `import module` after adding the package directory to sys.path
2. **Add proper sys.path configuration** - Include `sys.path.insert(0, os.path.expanduser('~/.hermes/mempalace'))` at the script's start
3. **Initialize components individually** - Rather than relying on package-level init (`from mempalace import init_mempalace`), call each component's init function directly (e.g., `capture.init_capture(path)`, `tag.init_tagging(path)`)
...
**Experience:** The Memory-Full Offload Procedure has been successfully executed multiple times as a scheduled cron job with no user present. Most recently on 2026-05-27, the procedure was validated during a scheduled cron job execution, confirming system operability and memory layer integration.
...
- **Maintenance script fixes:** When running maintenance scripts like `cron_maintenance.py` in cron jobs:
  - Convert relative imports to absolute imports (e.g., `from . import tag` → `import tag`)
  - Ensure proper sys.path configuration: `sys.path.insert(0, os.path.expanduser('~/.hermes/mempalace'))`
  - Initialize each component individually rather than relying on package-level init
  - Fix indentation issues that can cause control flow bugs (e.g., misplaced `if events is None:` blocks)
  - Verify variable name consistency (e.g., `embed._INDEX` vs `embed._index`)
...
**Latest Execution Results (2026-05-27):**
- Memory store capacity check: MEMORY.md: 307 chars, USER.md: 0 chars, Total: 307 chars (below 2,200 char threshold)
- Raw memories: 105
- Semantic memories: 7
- Episodic memories: 4
...
- ID map entries: 15
- Consolidation promoted: 0 memories (no events met threshold of 0.6)
- Pruning: 0 memories (no candidates met pruning criteria)
- Archive size: 0.0 MB
