...
### Generating Status Reports and Daily Briefings

Hermes provides several CLI commands that can be used to generate status reports and daily briefings:

- `hermes doctor` - Check system health and configuration
- `hermes cron list --all` - List all cron jobs (including disabled ones)
- `hermes cron status` - Check cron service status
- `hermes model` - Check current model/provider
- `hermes tools list` - Check enabled/disabled tools
- `hermes skills list` - List installed skills
- `hermes sessions list` - List recent sessions

Example workflow for generating a daily briefing:
1. Run `hermes doctor` to get overall system status and identify any issues
2. Run `hermes cron status` to check if the gateway and cron service are running
3. Run `hermes cron list --all` to get details on all scheduled jobs
4. Check on specific project status by examining relevant directories
5. Format the information into a concise bullet-point report

This approach was used to generate the daily briefing for Bob's "Tomorrow Remembered" book project.
...
Based on `hermes doctor` output, these maintenance tasks are commonly needed:

- **Config migration**: Run `hermes doctor --fix` or `hermes setup` to migrate outdated config
- **Install missing dependencies**: For better performance, install optional tools like ripgrep (`sudo apt install ripgrep`) for faster file search
- **Authenticate providers**: Run `hermes auth` to login to OAuth providers like Nous Portal, OpenAI Codex, etc.
