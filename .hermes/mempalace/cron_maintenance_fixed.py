...
def main():
    ...
    # Run consolidation (limit to avoid hanging)
    try:
    ...
    # Run pruning (dry run first to avoid hanging)
    try:
        print("\nRunning memory pruning (dry run)...")
        prune_stats = prune.prune_memories(dry_run=True)
        print(f"✓ Pruning dry run complete:")
