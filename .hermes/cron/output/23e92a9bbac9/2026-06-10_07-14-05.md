...
📌 **Hermes Agent Status**
- Doctor: 1 issue (run `hermes setup --fix` for missing API keys)
- Model: nvidia/nemotron-3-ultra-550b-a55b:free (provider: nous)
- Tools: 20 enabled, 6 disabled
- Skills: 148 enabled, 74 disabled
- Recent sessions: cron jobs dominate (reverse-tunnel-monitor, daily-briefing)

⏰ **Cron Job Health**
- Gateway running (PID: 240471), 22 active jobs
- Next run: 07:07 AM (reverse-tunnel-monitor)
- Issues: 
  • Pipeline Orchestrator: Telegram message too long
  • Backlink Acquisition: Response truncated
  • Model errors: 'nvidia/nemotron-3-ultra-550b-a55b:free' not found (affecting recent runs)

📚 **Book Project: "Tomorrow Remembered"**
- ✅ Published & in promotion phase (no book_source dir; output has tomorrow-remembered.epub)

⚠️ **System Alerts**
- Optional OAuths not logged in (Gemini, MiniMax, xAI)
- Docker & browser-cdp missing (optional)
- Memory plugin not found (run `hermes memory setup`)
- Model availability issue causing API failures

💡 **Insight/Reminder**
- Run `hermes doctor --fix` to resolve config/model issues. Verify model in `hermes config` for daily briefing job. Book is published—focus on promotion monitoring.
