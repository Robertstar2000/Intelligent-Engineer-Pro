...
- **Archive:** `/mnt/usb_4tb/backups/hermes-20260606-214711.tar.gz`
- **Size:** 3.5G
- **USB drive:** Mounted at `/mnt/usb_4tb` and used as backup destination.
- **Contents:** Includes `.hermes` (config, data, memories, skills, sessions, state database, etc.) and `~/books` (manuscripts, covers, EPUBs, PDFs). Excludes `hermes-agent/` repo, caches, node, checkpoints, sandboxes, bin.
- **Pruning:** Backups older than 30 days have been removed from `/mnt/usb_4tb/backups/`.

The backup procedure finished without errors. No further action required.
