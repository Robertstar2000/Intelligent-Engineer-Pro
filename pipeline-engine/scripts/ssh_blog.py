...
Uses pexpect for password-authenticated SSH/SCP
"""
...
def get_password():
    """Read DREAMHOST_PASSWORD from ~/.hermes/.env"""
...
def wp_publish(title, slug, category, tags, content_file, featured_image):
    """Publish to WordPress via PHP CLI"""
    # Note: double quotes inside single-quoted string for getopt()
    cmd = (
        f'cd {REMOTE_BASE} && '
    ...
    return ssh_run(cmd, timeout=180)
